               ...
               <form action="{{ route('dashboard.employees.update' , $employee->id)}}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        <select name="project_id" class="form-control" >
                            <option value="{{$employee->project_id}}">{{$employee->project->title}}</option>
                    ...
                    <div class="form-group">
                        ...
                        <select name="branch_id" class="form-control" >
                            <option value="{{$employee->branch_id}}">{{$employee->branch->title}}</option>
                    ...
                    <div class="form-group">
                        <img src="{{ asset($employee->image_path) }}" style="width: 100px" class="img-thumbnail image-preview" alt="">
                    </div>
                            ...
                            <div class="tab-content">
                                ...
                                @foreach ($models as $index=>$model)
                                    ...
                                    <div class="tab-pane {{ $index == 0 ? 'active' : ''}} " id="{{$model}}">
                                        ...
                                        @foreach ($maps as $map)
                                            <label><input type="checkbox" name="permissions[]" {{$employee->hasPermission($map.'_'.$model)? 'checked' : '' }} value="{{$map}}_{{$model}}">@lang('site.'.$map)</label>
                                        @endforeach
